...
class fiveGameSimulator:
    def __init__(self, model: fiveAlphaZero, UCB1: float, simulations: int) -> None:
        self.mcts = fiveMCTS(model, UCB1, sim_number=simulations)
    ...
    def game(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        game_state = fiveTicTacToeGameState.new_game(random.choice([1, -1]))
    ...
    def find_and_make_move(self,
                           game_state: fiveTicTacToeGameState) -> fiveTicTacToeGameState:
        move = self.mcts.find_move(game_state)
        game_state.make_move(move)
        return game_state

    def add_new_flipped_boards(self,
                               game_state: fiveTicTacToeGameState,
...
if __name__ == '__main__':
    game_simulator = fiveGameSimulator(fiveAlphaZero(), UCB1, SIMULATIONS)
