...
class LinearModel(nn.Module):
    ...
    def board2tensor(self, board: board_type, player: int,
                     device: torch.device) -> torch.Tensor:
        np_board = torch.from_numpy(board).to(device)
        ones = torch.ones((3, 3)).to(device)
        a = (np_board == ones).float().reshape(1, 9)
        b = (np_board == -ones).float().reshape(1, 9)
