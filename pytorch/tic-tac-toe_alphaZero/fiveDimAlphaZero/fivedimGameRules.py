...
class fiveTicTacToeGameState:
    ...
    def copy(self) -> 'fiveTicTacToeGameState':
        return fiveTicTacToeGameState(self.board.copy(), self.player, self.status)

    @staticmethod
    def new_game(starting_player: Optional[int] = None) -> 'fiveTicTacToeGameState':
        board = np.zeros((3, 3), dtype=np.int8)
        starting_player = starting_player if starting_player else -1
        return fiveTicTacToeGameState(board, starting_player)
    ...
    def calculate_game_status(self) -> int:
        ...
        if middle == 0:
            return fiveTicTacToeGameState.GAME_NOT_OVER
        ...
        if not (self.board == 0).any():
            return fiveTicTacToeGameState.DRAW

        return fiveTicTacToeGameState.GAME_NOT_OVER

    def game_over(self) -> bool:
        if self.status == fiveTicTacToeGameState.GAME_NOT_OVER:
            return False
        return True
    ...
    def move2index(move) -> int:
        return fiveTicTacToeGameState.MOVE2INDEX[move]
    ...
    def get_masks(boards: torch.Tensor) -> torch.Tensor:
        ...
        player_one = boards[:, 1] == ones
        player_two = boards[:, 2] == ones
