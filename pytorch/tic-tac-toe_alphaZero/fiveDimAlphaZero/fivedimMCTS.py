...
from fiveDimAlphaZero.fivedimNode import fiveNode
from fiveDimAlphaZero.fivedimTicTacToeModel import fiveAlphaZero


class fiveMCTS:
    def __init__(self, model: fiveAlphaZero, exploration_rate: float,
    ...
    def find_move(self, game_state: fiveTicTacToeGameState) -> tuple[int, int]:
        self.root = fiveNode(game_state)
    ...
    def best_node_found(self, current: fiveNode) -> bool:
        return current.visits > 0.5*self.sim_number

    def traverse_tree(self) -> fiveNode:
        current = self.root
    ...
    def expand_tree(self, current: fiveNode) -> fiveNode:
    ...
    def evaluate_board(self, current: fiveNode) -> tuple[torch.Tensor, torch.Tensor]:
        with torch.no_grad():
    ...
    def add_noise_if_root(self, current: fiveNode, policy: torch.Tensor) -> torch.Tensor:
    ...
    def mask_illegal_moves(self, torch_board: torch.Tensor,
        ...
        mask = fiveTicTacToeGameState.get_masks(torch_board)
        return policy * mask
    ...
    def rollout(self, current: fiveNode):
