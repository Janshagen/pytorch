...
class fiveNode:
    def __init__(self, game_state: fiveTicTacToeGameState,
                 move: Optional[tuple[int, int]] = None,
                 prior: float = 0,
                 parent: Optional['fiveNode'] = None
                 ) -> None:
        # game_state after move has been made
        self.game_state = game_state

        self.move: Optional[tuple[int, int]] = move
        self.parent: Optional['fiveNode'] = parent

        self.children: list['fiveNode'] = []
    ...
    def make_children(self, policy: torch.Tensor) -> None:
        ...
        for move in moves:
            ...
            prior = policy[fiveTicTacToeGameState.move2index(move)].item()

            child = fiveNode(state, move, prior, parent=self)
            self.children.append(child)

    def select_child(self, C: float) -> 'fiveNode':
    ...
    def calculate_PUCT(self, C: float, child: 'fiveNode') -> float:
    ...
    def tree_traverse(self, max_level: int) -> list[str]:
        to_be_traversed: list[tuple['fiveNode', int, str]] = [(self, 0, " ")]
        instructions = ["first"]
        previous_level = 0
