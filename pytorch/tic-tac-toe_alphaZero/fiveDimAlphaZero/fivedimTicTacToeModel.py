...
class fiveAlphaZero(nn.Module):
    ...
    MODEL_PATH = \
        '/home/anton/skola/egen/pytorch/tic-tac-toe_alphaZero/fiveDimAlphaZero/models/'
    ...
    def state2tensor(self, game_state: fiveTicTacToeGameState) -> torch.Tensor:
    ...
    def get_load_file(file: Optional[str] = None) -> str:
        if file:
            return fiveAlphaZero.MODEL_PATH + file

        model_name = re.compile(".*?AlphaZero(.*?).pth")
        files = os.listdir(fiveAlphaZero.MODEL_PATH)
        ...
        return fiveAlphaZero.MODEL_PATH + files[newest_index]
