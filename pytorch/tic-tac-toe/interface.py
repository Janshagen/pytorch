...
def draw(screen: pygame.Surface, frame: pygame.Surface, board: np.ndarray, WIDTH: int, move: int = None, player: int = 0) -> None:
    # if type(move) == int:
...
def drawPieces(screen: pygame.Surface, board: np.ndarray, player, WIDTH: int) -> None:
    # placed pieces
...
def int2coord(i: int, j: int, w: int) -> np.ndarray:
    return np.array([w*i + w//2, w*j + w//2])
...
def mousePos(WIDTH: int):
    ...
    for i in range(7):
        for j in range(7):
            if WIDTH * i < mPos[1] < WIDTH * (i+1) and WIDTH * j < mPos[0] < WIDTH * (j+1):
                return (i, j)
    return None, None
